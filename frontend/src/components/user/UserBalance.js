    ...
    return (
        <div>
            <div className="element">
                ...
                <span style={{color: 'green'}}>{fromUnitsToDecimal(vaultStore.balanceOf.value, vaultStore.decimals)}</span></label>
            </div>
            <div className="row main-container">
                <div className="col-6">
                    <div className="element">                    
                        <label className="paste-label" style={{textAlign: 'center', width: "100%"}}>Your balance {tokenStore.symbolToken0}:
                        <span style={{color: 'green'}}>{truncateNumber(fromUnitsToDecimal(userToken0, tokenStore.decimalsToken0),4)}</span></label>
                    </div>
                </div>
                <div className="col-6">
                    <div className="element">
                        <label className="paste-label" style={{textAlign: 'center', width: "100%"}}>Your balance {tokenStore.symbolToken1}:
                        <span style={{color: 'green'}}>{truncateNumber(fromUnitsToDecimal(userToken1, tokenStore.decimalsToken1), 4)}</span></label>
                    </div>
                </div>
            </div>
        </div>
